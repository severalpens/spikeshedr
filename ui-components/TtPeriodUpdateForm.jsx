...
import { getTtPeriod } from "./graphql/queries";
import { updateTtPeriod } from "./graphql/mutations";
const client = generateClient();
export default function TtPeriodUpdateForm(props) {
  const {
    id: idProp,
    ttPeriod: ttPeriodModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = ttPeriodRecord
      ? { ...initialValues, ...ttPeriodRecord }
      : initialValues;
  ...
  const [ttPeriodRecord, setTtPeriodRecord] = React.useState(ttPeriodModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTtPeriod.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTtPeriod
        : ttPeriodModelProp;
      setTtPeriodRecord(record);
    };
    queryData();
  }, [idProp, ttPeriodModelProp]);
  React.useEffect(resetStateValues, [ttPeriodRecord]);
  const validations = {
    StartTime: [],
    EndTime: [],
    TimerTaskId: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTtPeriod.replaceAll("__typename", ""),
            variables: {
              input: {
                id: ttPeriodRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TtPeriodUpdateForm")}
      {...rest}
    >
      <TextField
        label="Start time"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || ttPeriodModelProp)}
